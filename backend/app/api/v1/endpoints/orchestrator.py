...
from app.orchestrator.core import (OrchestratorAgent, ProcessingStatus,
                                   QueryContext, QueryType)
from app.schemas.orchestrator import (BatchProcessingResponse,
                                      BatchQueryRequest, DailyBriefRequest,
                                      HealthStatusResponse, QueryRequest,
                                      QueryResponse, StatisticsResponse,
                                      TaskStatusResponse)
from app.tasks.celery_app import celery_app
from app.tasks.intelligence_tasks import (get_task_status,
                                          health_check_all_modules,
                                          process_daily_brief, process_query,
...
async def process_intelligence_query(
    ...
    try:
        logger.info(
            f"Processing query from user {current_user['id']}: {request.query[:50]}..."
        )
    ...
    try:
        ...
        for module_name in modules:
            ...
            if module:
                module_info[module_name] = {
                    ...
                    "available": orchestrator.module_registry.is_module_available(
                        module_name
                    ),
                }
        ...
        return {
            ...
            "available_modules": len(
                [m for m in module_info.values() if m["available"]]
            ),
        }
...
async def process_batch_queries(
    request: BatchQueryRequest, current_user: Dict[str, Any] = Depends(get_current_user)
    ...
    try:
        ...
        logger.info(
            f"Processing batch of {len(queries)} queries for user {current_user['id']}"
        )
