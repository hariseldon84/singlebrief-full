...
from app.auth.dependencies import (get_client_ip, get_current_active_user,
                                   get_current_user, get_user_agent,
                                   log_login_attempt, verify_refresh_token)
from app.auth.oauth import extract_user_data_from_oauth, get_oauth_provider
from app.core.database import get_db_session, get_redis
from app.core.security import (create_access_token,
                               create_email_verification_token,
                               create_refresh_token,
                               generate_password_reset_token, hash_password,
                               verify_email_verification_token,
                               verify_password)
from app.models.auth import (EmailVerificationToken, LoginAttempt,
                             PasswordResetToken, RefreshToken)
from app.models.user import Organization, User, UserRole
from app.schemas.auth import (EmailVerification, OAuthLogin, PasswordChange,
                              PasswordReset, RefreshTokenRequest,
                              ResendVerification, Token, TokenResponse,
                              UserLogin, UserRegister)
from app.schemas.user import OrganizationResponse, UserResponse
...
async def register_user(
    ...
    try:
        ...
        return TokenResponse(
            ...
            organization=(
                OrganizationResponse.from_orm(organization) if organization else None
            ),
        )
...
async def login_user(
    user_data: UserLogin, request: Request, db: AsyncSession = Depends(get_db_session)
):
    ...
    try:
        ...
        if user.is_locked:
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Account is locked"
            )
        ...
        return TokenResponse(
            ...
            organization=(
                OrganizationResponse.from_orm(user.organization)
...
async def refresh_access_token(
    token_data: RefreshTokenRequest, db: AsyncSession = Depends(get_db_session)
):
...
async def verify_email(
    verification: EmailVerification, db: AsyncSession = Depends(get_db_session)
):
...
async def get_oauth_url(provider: str, redirect_uri: str, state: str = None):
