...
class ContextRetrievalRequest(BaseModel):
    ...
    max_memories: int = Field(10, ge=1, le=20, description="Maximum memories to retrieve")
    max_conversations: int = Field(3, ge=1, le=10, description="Maximum conversations to retrieve")
    max_decisions: int = Field(5, ge=1, le=10, description="Maximum decisions to retrieve")
    time_window_days: int = Field(30, ge=1, le=365, description="Time window for context retrieval")


class MemoryLifecycleRequest(BaseModel):
    operation: str = Field(..., description="Lifecycle operation: expiration, optimization, cleanup")
...
async def manage_memory_lifecycle(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Lifecycle management failed: {str(e)}")


@router.get("/usage-report", response_model=Dict[str, Any])
async def get_memory_usage_report(
    organization_id: Optional[str] = Query(None, description="Organization ID for scoped report"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Usage report generation failed: {str(e)}")
...
async def delete_memory(
    memory_id: str,
    memory_type: str = Query(..., description="Memory type: user_memory or team_memory"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
