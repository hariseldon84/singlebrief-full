...
async def analyze_user_preferences(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze user preferences: {str(e)}"
        )
...
async def get_user_preferences(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve user preferences: {str(e)}"
        )
...
async def get_other_user_preferences(
    ...
    if not current_user.is_admin:
        if (
            ...
            != await _get_user_organization(session, user_id)
            or current_user.role != "team_lead"
        ):
            raise HTTPException(
                status_code=403, detail="Not authorized to view this user's preferences"
            )
...
async def get_communication_style_analysis(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze communication style: {str(e)}"
        )
...
async def get_topic_interests_analysis(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze topic interests: {str(e)}"
        )
