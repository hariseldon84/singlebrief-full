...
class ConsentGrantRequest(BaseModel):
    ...
    consent_type: str = Field(..., description="Type: memory_storage, preference_learning, team_sharing, analytics, data_export")
    consent_scope: str = Field(..., description="Scope: personal_memory, team_memory, cross_team, external_sharing")
    legal_basis: str = Field(..., description="GDPR legal basis")
    processing_purpose: str = Field(..., description="Purpose of data processing")
    consent_method: str = Field(default="explicit", description="How consent was obtained")
    expires_in_days: Optional[int] = Field(None, description="Days until consent expires")
    consent_evidence: Optional[Dict[str, Any]] = Field(None, description="Evidence of consent")
...
class ConsentWithdrawalRequest(BaseModel):
    ...
    withdrawal_evidence: Optional[Dict[str, Any]] = Field(None, description="Evidence of withdrawal")


class DataExportRequest(BaseModel):
    """Request model for data export."""
    export_type: str = Field(..., description="Type: full_export, partial_export, specific_data, gdpr_export")
    data_categories: List[str] = Field(..., description="Categories: conversations, user_memories, preferences, decisions, team_memories")
    export_format: str = Field(default="json", description="Format: json, csv, xml, pdf")
    ...
    additional_filters: Optional[Dict[str, Any]] = Field(None, description="Additional filters")
...
async def request_data_export(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create data export request: {str(e)}"
        )
...
async def download_export_file(
    ...
    try:
        ...
        if export_request.download_expires_at and export_request.download_expires_at < datetime.now():
...
async def delete_all_user_data(
    ...
    try:
        ...
        from app.models.memory import (
            UserMemory, UserPreference, UserBehaviorPattern,
            Conversation, Decision, PrivacyConsent
        )
        ...
        await session.execute(
            delete(UserBehaviorPattern).where(UserBehaviorPattern.user_id == current_user.id)
        )
