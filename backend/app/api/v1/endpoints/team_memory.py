...
class TeamMemoryCreateRequest(BaseModel):
    ...
    memory_type: str = Field(
        ...,
        description="Type: team_process, decision_pattern, collaboration_style, project_context",
    )
    category: str = Field(..., min_length=1, max_length=100)
    metadata: Optional[Dict[str, Any]] = None
    visibility: str = Field(
        default="team", description="Visibility: team, organization, private"
    )
    access_level: str = Field(
        default="read_write", description="Access: read_only, read_write, admin_only"
    )
...
class CrossTeamSharingRequest(BaseModel):
    ...
    sharing_permissions: Dict[str, Any] = Field(
        ..., description="Sharing permissions and controls"
    )
...
async def create_team_memory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create team memory: {str(e)}"
        )
...
async def get_team_memories(
    ...
    min_consensus: Optional[float] = Query(
        None, ge=0.0, le=1.0, description="Minimum consensus level"
    ),
    limit: int = Query(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve team memories: {str(e)}"
        )
...
async def validate_team_memory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to validate team memory: {str(e)}"
        )
...
async def analyze_team_interactions(
    ...
    try:
        team_collaboration_service.session = session
        analysis = await team_collaboration_service.analyze_team_interaction_patterns(
            team_id
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze team interactions: {str(e)}"
        )
...
async def update_memory_permissions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update memory permissions: {str(e)}"
        )

@router.post(
    "/teams/{source_team_id}/memories/{memory_id}/share", response_model=Dict[str, Any]
)
async def share_memory_across_teams(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to share memory across teams: {str(e)}"
        )
...
async def get_team_health_indicators(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get team health indicators: {str(e)}"
        )
...
async def get_memory_categories(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get memory categories: {str(e)}"
        )
