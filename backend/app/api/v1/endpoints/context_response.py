...
class ContextualQueryRequest(BaseModel):
    ...
    response_format: Optional[str] = Field(
        None,
        description="Preferred response format: conversational, structured, bullet_points",
    )
    include_team_context: bool = Field(
        True, description="Whether to include team memory context"
    )
    include_proactive_insights: bool = Field(
        True, description="Whether to include proactive insights"
    )
    include_follow_ups: bool = Field(
        True, description="Whether to include follow-up suggestions"
    )
...
class ResponseFormattingRequest(BaseModel):
    ...
    format_type: str = Field(
        ..., description="Format type: conversational, structured, bullet_points"
    )
    tone: Optional[str] = Field(
        None, description="Response tone: professional, casual, direct"
    )
    detail_level: Optional[str] = Field(
        None, description="Detail level: brief, medium, detailed"
    )
...
async def generate_contextual_response(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate contextual response: {str(e)}"
        )
...
async def format_response_content(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to format response: {str(e)}"
        )
...
async def get_user_context_summary(
    ...
    if current_user.id != user_id and not current_user.is_admin:
        if current_user.organization_id != await _get_user_organization(
            session, user_id
        ) or current_user.role not in ["team_lead", "admin"]:
            raise HTTPException(
                status_code=403, detail="Not authorized to access this user's context"
            )
    ...
    try:
        ...
        user_preferences = (
            await context_response_service._get_user_response_preferences(
                session, user_id
            )
        )
        ...
        return {
            "user_id": user_id,
            "context_summary": {
                "memory_count": context.get("user_memory_context", {}).get(
                    "total_memories", 0
                ),
            ...
            "communication_preferences": user_preferences.get(
                "communication_style", {}
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get user context: {str(e)}"
        )
...
async def get_response_style_preferences(
    ...
    try:
        ...
        return {
            ...
            "preference_summary": {
                "communication_style_set": len(
                    preferences.get("communication_style", {})
                )
                > 0,
                "format_preferences_set": len(preferences.get("format_preferences", {}))
                > 0,
                "confidence_range": {
                    "min": (
                        min(preferences.get("confidence_levels", {}).values())
                        if preferences.get("confidence_levels")
                        else 0
                    ),
                    "max": (
                        max(preferences.get("confidence_levels", {}).values())
                        if preferences.get("confidence_levels")
                        else 0
                    ),
                    "avg": sum(preferences.get("confidence_levels", {}).values())
                    / max(len(preferences.get("confidence_levels", {})), 1),
                },
            },
        }
...
async def interpret_query_with_context(
    ...
    try:
        ...
        interpretation = {
            ...
            "context_enhanced": {
                "has_historical_context": len(
                    context.get("user_memory_context", {}).get("memories_by_type", {})
                )
                > 0,
                "has_team_context": len(context.get("team_context", {})) > 0,
                "has_conversation_context": len(context.get("conversation_context", {}))
                > 0,
                "relevant_patterns": context.get("historical_patterns", {}).get(
                    "patterns_found", 0
                ),
                "recent_decisions_relevant": len(
                    context.get("recent_decisions", {}).get("decisions", [])
                ),
            },
            ...
            "context_disambiguation": self._generate_disambiguation_options(
                query, context
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to interpret query: {str(e)}"
        )

def _calculate_interpretation_confidence(
    query_context: Dict[str, Any], context: Dict[str, Any]
) -> float:
...
def _generate_clarification_suggestions(
    ...
    # If query is vague
    if (
        query_context.get("complexity") == "low"
        and len(query_context.get("detected_topics", [])) == 0
    ):
        suggestions.append(
    ...
    if (
        query_context.get("urgency_level") == "normal"
        and "important" in query_context.get("query", "").lower()
    ):
