...
class QueryIntent(BaseModel):
    ...
    entities: List[Dict[str, str]] = Field(
        default=[], description="Named entities found"
    )
    time_context: Optional[str] = Field(default=None, description="Time context if any")
    urgency: str = Field(
        default="normal", description="Urgency level: low, normal, high, urgent"
    )
    scope: str = Field(
        default="team", description="Scope: personal, team, organization, project"
    )
    query_type: str = Field(
        description="Type of query: status, update, question, command"
    )
...
class QueryParser:
    ...
    async def parse(self, query: str, context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Parsed query with intent: {final_intent.get('primary_intent')}"
            )
            return parsed_query
    ...
    def _detect_intent_patterns(self, query: str) -> Dict[str, Any]:
        ...
        return {
            "primary_intent": (
                detected_intents[0] if detected_intents else "general_query"
            ),
            "secondary_intents": (
                detected_intents[1:] if len(detected_intents) > 1 else []
            ),
...
"""
    ...
    def _merge_intent_analysis(
        self, pattern_result: Dict[str, Any], llm_result: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _determine_modules(self, intent: Dict[str, Any]) -> List[str]:
        ...
        modules.update(
            self.intent_to_modules.get(primary_intent, ["team_comms_crawler"])
        )
