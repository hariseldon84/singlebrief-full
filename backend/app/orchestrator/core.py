...
"""
    ...
    async def process_query(
        ...
        try:
            ...
            logger.info(f"Query {context.query_id} completed in {result.processing_time_ms}ms")
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        unhealthy_modules = [
            name for name, module_status in status['modules'].items()
            if module_status.get('status') != 'healthy'
        ]
        
        if unhealthy_modules:
            status['orchestrator'] = 'degraded' if len(unhealthy_modules) < len(status['modules']) / 2 else 'unhealthy'
