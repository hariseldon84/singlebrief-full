...
class ResponseSynthesizer:
    ...
    async def _analyze_module_results(self, module_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for module_name, result in module_results.get('data', {}).items():
            ...
            if 'confidence_metrics' in result.get('data', {}):
                confidence = result['data']['confidence_metrics'].get('overall_confidence', 0.5)
            ...
            analysis['data_quality'][module_name] = {
                ...
                'completeness': min(module_source_count / 5, 1.0)  # Assume 5 sources is "complete"
            }
    ...
    def _check_data_freshness(self, result: Dict[str, Any]) -> bool:
        ...
        return any(
            ...
            for value in data.values() if isinstance(value, (str, list))
        )
    ...
    def _determine_synthesis_strategy(
        ...
        if any(word in query_lower for word in ['action', 'do', 'next steps', 'recommend']):
    ...
    def _prepare_data_summary(self, module_results: Dict[str, Any]) -> str:
        ...
        for module_name, result in module_results.get('data', {}).items():
            ...
            if sources:
                ...
                source_types = list(set(source.get('type', 'unknown') for source in sources))
...
"""
        ...
        return {
            ...
            'confidence': 0.3,
            'key_insights': [],
            'action_items': ['Please try your query again for a more detailed response'],
