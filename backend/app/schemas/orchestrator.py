...
class QueryRequest(BaseModel):
    ...
    priority: QueryPriority = Field(
        default=QueryPriority.NORMAL, description="Query priority level"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "context": {"session_id": "sess_abc123", "source": "web_ui"},
                "priority": 5,
...
class BatchQueryRequest(BaseModel):
    ...
    queries: List[QueryRequest] = Field(
        ..., description="List of queries to process", max_items=10
    )
    batch_priority: QueryPriority = Field(
        default=QueryPriority.NORMAL, description="Priority for the entire batch"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                "queries": [
                    {"query": "Team status update", "priority": 5},
                    {"query": "Project milestone progress", "priority": 7},
                ],
                "batch_priority": 5,
            }
        }
...
class DailyBriefRequest(BaseModel):
    ...
    include_sentiment: bool = Field(
        default=True, description="Include team sentiment analysis"
    )
...
class SourceInfo(BaseModel):
    ...
    confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Source confidence score"
    )
...
class QueryResponse(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Overall confidence in the response"
    )
    ...
    modules_used: List[str] = Field(
        default_factory=list, description="Orchestrator modules involved"
    )
    ...
    key_insights: List[str] = Field(
        default_factory=list, description="Key insights extracted from the response"
    )
...
class TaskStatusResponse(BaseModel):
    ...
    progress: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Completion progress"
    )
...
class StatisticsResponse(BaseModel):
    ...
    total_queries_processed: int = Field(
        ..., ge=0, description="Total queries processed"
    )
    ...
    most_common_query_types: List[Dict[str, Any]] = Field(
        default_factory=list, description="Most common query types with counts"
    )
    performance_metrics: Dict[str, float] = Field(
        default_factory=dict, description="Performance percentiles"
    )
...
class BatchProcessingResponse(BaseModel):
    ...
    estimated_completion_seconds: int = Field(
        ..., ge=0, description="Estimated completion time"
    )
