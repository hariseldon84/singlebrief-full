    ...
    try:
        ...
        for channel in channels:
            try:
                ...
                logger.error(
                    f"Failed to deliver notification via {channel}: {channel_exc}"
                )
                delivery_results.append(
                    {"channel": channel, "status": "failed", "error": str(channel_exc)}
                )
        ...
        notification_result = {
            ...
            "failed_deliveries": len(
                [r for r in delivery_results if r["status"] == "failed"]
            ),
...
def send_daily_brief_notification(
    self, user_id: str, brief_summary: str, brief_url: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        return send_notification.apply(
            args=[
                ...
                message,
                user_channels,
                {"brief_url": brief_url, "brief_type": "daily", "auto_generated": True},
            ],
            task_id=self.request.id,
        ).get()
...
@celery_app.task(
    bind=True, base=NotificationTask, name="app.tasks.notification_tasks.send_alert"
)
...
def send_alert(
    ...
    try:
        ...
        logger.info(
            f"Sending {severity} alert to {len(affected_users)} users in org {organization_id}"
        )
        ...
        for user_id in affected_users:
            try:
                ...
                alert_results.append(
                    {"user_id": user_id, "status": "failed", "error": str(user_exc)}
                )
        ...
        return {
            ...
            "notification_failures": len(
                [r for r in alert_results if r["status"] == "failed"]
            ),
...
def send_integration_status(
    self, organization_id: str, integration_name: str, status: str, details: str
) -> Dict[str, Any]:
    """Send integration status notifications"""
    try:
        logger.info(
            f"Sending integration status notification for {integration_name} in org {organization_id}"
        )
        ...
        if status in ["error", "warning"]:
            ...
            message = (
                f"Your {integration_name} integration status is {status}. {details}"
            )
    ...
    try:
        ...
        return {
            ...
            "successful_sends": len(
                [r for r in batch_results if r["status"] == "sent"]
            ),
