...
@celery_app.task(
    bind=True, base=IntegrationTask, name="app.tasks.integration_tasks.sync_slack_data"
)
...
def sync_slack_data(
    ...
    try:
        ...
        logger.info(
            f"Slack sync completed for org {organization_id}: {sync_result['messages_synced']} messages"
        )
        return sync_result
...
@celery_app.task(
    bind=True, base=IntegrationTask, name="app.tasks.integration_tasks.sync_email_data"
)

def sync_email_data(
    self, organization_id: str, user_id: str, email_provider: str = "gmail"
) -> Dict[str, Any]:
...
def sync_calendar_data(
    self, organization_id: str, user_id: str, calendar_provider: str = "google"
) -> Dict[str, Any]:
...
@celery_app.task(
    bind=True, base=IntegrationTask, name="app.tasks.integration_tasks.sync_github_data"
)

def sync_github_data(
    self, organization_id: str, repository_urls: List[str]
) -> Dict[str, Any]:
...
def sync_all_integrations(self) -> Dict[str, Any]:
    ...
    try:
        ...
        organizations = [
            ...
            {"id": "org_2", "integrations": ["slack", "calendar"], "teams": ["team_3"]},
        ]
        ...
        for org in organizations:
            ...
            try:
                ...
                sync_results.append(
                    {"org_id": org_id, "status": "failed", "error": str(org_exc)}
                )
...
def validate_integration(
    ...
    try:
        logger.info(
            f"Validating {integration_type} integration for org {organization_id}"
        )
...
def cleanup_stale_data(
    self, organization_id: str, integration_type: str, days_old: int = 30
) -> Dict[str, Any]:
    """Clean up stale integration data"""
    try:
        logger.info(
            f"Cleaning up stale {integration_type} data for org {organization_id}"
        )
...
def integration_health_check(self, organization_id: str) -> Dict[str, Any]:
