...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.update_memory"
)
def update_memory(
    self,
    user_id: str,
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.consolidate_memories"
)
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            if apply_expiration:
                ...
                expiration_result = await memory_lifecycle_manager.apply_expiration_policies(
                    db, organization_id, dry_run=False
                )
                consolidation_result['operations_performed'].append('expiration_policies')
                consolidation_result['expiration_result'] = expiration_result
                consolidation_result['total_items_processed'] += expiration_result['expired_counts']['total_items']

            # Optimize storage
            if optimize_storage:
                logger.info("Optimizing memory storage")
                optimization_result = await memory_lifecycle_manager.optimize_memory_storage(
                    db, organization_id
                )
                consolidation_result['operations_performed'].append('storage_optimization')
                consolidation_result['optimization_result'] = optimization_result
                consolidation_result['storage_freed_mb'] += optimization_result['optimization_results']['storage_freed_mb']

            # Cleanup orphaned embeddings
            if cleanup_orphaned:
                logger.info("Cleaning up orphaned embeddings")
                cleanup_result = await memory_lifecycle_manager.cleanup_orphaned_embeddings(
                    db, organization_id
                )
                consolidation_result['operations_performed'].append('orphaned_cleanup')
                consolidation_result['cleanup_result'] = cleanup_result
                consolidation_result['total_items_processed'] += cleanup_result['cleanup_counts']['orphaned_embeddings']
        ...
        logger.info(f"Memory consolidation completed: {consolidation_result['total_items_processed']} items processed")
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.extract_insights"
)
def extract_insights(
    ...
    try:
        logger.info(f"Extracting insights for user {user_id} over {time_period_days} days")
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.update_user_preferences"
)
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.generate_context"
)
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.backup_memory"
)
def backup_memory(
    self,
    organization_id: str,
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.analyze_memory_usage"
)
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.refresh_memory_indexes"
)
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.apply_memory_expiration"
)
async def apply_memory_expiration(
    ...
    try:
        logger.info(f"Applying memory expiration for org {organization_id}, dry_run={dry_run}")
...
@celery_app.task(
    bind=True,
    base=MemoryTask,
    name="app.tasks.memory_tasks.optimize_memory_storage"
)
