...
def process_query(
    self, query: str, context_data: Dict[str, Any], priority: int = 5
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying query processing task (attempt {self.request.retries + 1})"
            )
...
def process_urgent_query(
    ...
    return process_query.apply(
        args=[query, context_data, 9], task_id=self.request.id  # Priority 9 for urgent
    ).get()
...
def process_regular_query(
    ...
    return process_query.apply(
        args=[query, context_data, 5], task_id=self.request.id  # Priority 5 for regular
    ).get()
...
def process_daily_brief(
    self, user_id: str, organization_id: str, team_id: Optional[str] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        for user in active_users:
            try:
                ...
                logger.error(
                    f"Failed to queue daily brief for user {user['user_id']}: {user_exc}"
                )
                generated_briefs.append(
                    {
                        "user_id": user["user_id"],
...
def health_check_all_modules(self) -> Dict[str, Any]:
    ...
    try:
        ...
        asyncio.set_event_loop(loop)
        try:
            health_status = loop.run_until_complete(orchestrator.get_health_status())
        finally:
            loop.close()
...
def batch_process_queries(self, queries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        for query_data in queries:
            try:
                ...
                # Process query
                result = process_query.apply(args=[query, context_data, priority]).get()
