...
from app.models.memory import (Conversation, Decision, TeamMemory,
...
class ContextAwareResponseService:
    ...
    async def generate_contextualized_response(
        ...
        try:
            ...
            user_preferences = await self._get_user_response_preferences(
                session, user_id
            )
    ...
    async def _build_comprehensive_context(
        ...
        context = {
            ...
            "conversation_context": (
                await self._get_conversation_context(session, conversation_id)
                if conversation_id
                else {}
            ),
            "user_memory_context": await self._get_relevant_user_memories(
                session, user_id, query
            ),
            "team_context": (
                await self._get_relevant_team_context(session, user_id, query)
                if include_team_context
                else {}
            ),
            "historical_patterns": await self._get_user_interaction_patterns(
                session, user_id, query
            ),
            "recent_decisions": await self._get_recent_decisions(session, user_id),
            "topic_preferences": await self._get_topic_preferences(
                session, user_id, query
            ),
        }
    ...
    async def _analyze_query_context(self, query: str) -> Dict[str, Any]:
        ...
        is_question = any(
            word in query_lower for word in question_words
        ...
        urgency_level = (
            "high" if any(word in query_lower for word in urgent_words) else "normal"
        )
        ...
        complexity = (
            "high" if word_count > 20 else "medium" if word_count > 10 else "low"
        )
    ...
    async def _get_conversation_context(
        ...
        recent_messages = sorted(
            conversation.messages, key=lambda m: m.sequence_number
        )[-10:]
    ...
    async def _get_relevant_user_memories(
        self, session: AsyncSession, user_id: str, query: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_importance": sum(m.importance_score for m in memories)
            / max(len(memories), 1),
            "date_range": {
    ...
    async def _get_relevant_team_context(
        ...
        teams_query = select(TeamMember.team_id).where(
            and_(TeamMember.user_id == user_id, TeamMember.is_active.is_(True))
        )
    ...
    async def _get_user_interaction_patterns(
        ...
        patterns_query = (
            ...
            .where(
                and_(
                    ...
                    UserBehaviorPattern.is_active.is_(True),
                    UserBehaviorPattern.predictive_value >= 0.3,
                )
            )
            .order_by(UserBehaviorPattern.predictive_value.desc())
            .limit(5)
        )
        ...
        return {"patterns_found": len(relevant_patterns), "patterns": relevant_patterns}
    ...
    async def _get_recent_decisions(
        ...
        decisions_query = (
            select(Decision)
            .where(
                and_(Decision.user_id == user_id, Decision.decided_at >= cutoff_date)
            )
    ...
    async def _determine_response_structure(
        ...
        response_structure = {
            ...
            "personalization_elements": self._get_personalization_elements(
    ...
    def _determine_tone(
        self, comm_style: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
    ...
    def _determine_detail_level(
        self, comm_style: Dict[str, Any], query: str, context: Dict[str, Any]
    ) -> str:
    ...
    def _determine_technical_depth(
        self, comm_style: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
    ...
    def _get_personalization_elements(
        ...
        elements = {
            "include_historical_context": len(
                context.get("user_memory_context", {}).get("memories_by_type", {})
            )
            > 0,
            "reference_recent_decisions": len(
                context.get("recent_decisions", {}).get("decisions", [])
            )
            > 0,
            "acknowledge_patterns": len(
                context.get("historical_patterns", {}).get("patterns", [])
            )
            > 0,
    ...
    def _get_adaptation_suggestions(self, context: Dict[str, Any]) -> List[str]:
        ...
        for pattern in patterns:
            if pattern.get("predictive_value", 0) > 0.7:
                suggestions.append(
                    f"Based on your {pattern['pattern_type']} pattern, consider..."
                )
    ...
    async def _generate_follow_up_suggestions(
        self, session: AsyncSession, user_id: str, query: str, context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for topic in detected_topics:
            if topic in topic_suggestions:
                for suggestion in topic_suggestions[topic][:2]:  # Limit to 2 per topic
                    suggestions.append(
                        {"question": suggestion, "category": topic, "relevance": "high"}
                    )
    ...
    async def _generate_proactive_insights(
        self, session: AsyncSession, user_id: str, context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if team_context.get("relevant_memories"):
            high_consensus_memories = [
                m
                for m in team_context["relevant_memories"]
        ...
        user_memories = context.get("user_memory_context", {}).get(
            "memories_by_type", {}
        )
    ...
    async def apply_response_formatting(
        ...
        formatted_response = {
            ...
            "personalization_applied": response_structure.get(
                "personalization_elements", {}
            ),
            "formatting_metadata": {
                "response_length": len(response_content),
                "word_count": len(response_content.split()),
                "estimated_reading_time": len(response_content.split())
                // 200,  # ~200 WPM
            },
        }
        ...
        if response_structure.get("format") == "structured":
            formatted_response["structured_content"] = self._create_structured_format(
                response_content
            )
        elif response_structure.get("format") == "bullet_points":
            formatted_response["bullet_points"] = self._create_bullet_format(
                response_content
            )
