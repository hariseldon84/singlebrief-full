...
from app.models.memory import (Conversation, ConversationMessage,
                               UserBehaviorPattern, UserPreference)
...
class PreferenceLearningService:
    ...
    async def analyze_communication_style(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._update_communication_preferences(
                session, user_id, style_analysis
            )
    ...
    def _analyze_formality(self, messages: List[ConversationMessage]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence": min(
                total_indicators / 10, 1.0
            ),  # Max confidence at 10+ indicators
        }

    def _analyze_length_preference(
    ...
    def _analyze_question_style(
        ...
        style = (
            "exploratory"
            if open_ratio > 0.6
            else "direct" if open_ratio < 0.4 else "balanced"
        )
    ...
    async def _update_communication_preferences(
        self, session: AsyncSession, user_id: str, analysis: Dict[str, Any]
    ) -> None:
    ...
    async def analyze_topic_interests(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "analysis": combined_analysis}
    ...
    def _analyze_topic_engagement(
        ...
        engagement_metrics = {
            "average_message_length": statistics.mean(
                [len(msg.content.split()) for msg in messages]
            ),
            "question_frequency": sum(
                1 for msg in messages if msg.content.strip().endswith("?")
            )
            / len(messages),
    ...
    async def _upsert_preference(
        ...
        if existing_pref:
            ...
            existing_pref.confidence_score = max(
                existing_pref.confidence_score, confidence
            )
            ...
            if existing_pref.evidence_count > 3:
                existing_pref.stability_score = min(
                    existing_pref.stability_score + 0.1, 1.0
                )
        else:
    ...
    async def update_preference_manually(
        self, user_id: str, category: str, key: str, value: Any, organization_id: str
    ) -> None:
        ...
        try:
            ...
            results = {
                ...
                "communication_style": (
                    ...
                    if not isinstance(communication_results, Exception)
                    else {"error": str(communication_results)}
                ),
                "topic_interests": (
                    topic_results
                    if not isinstance(topic_results, Exception)
                    else {"error": str(topic_results)}
                ),
            }
            ...
            logger.error(
                f"Error running preference learning analysis for user {user_id}: {e}"
            )
            raise
