...
from app.models.memory import TeamMemory, UserMemory, ConversationMessage, Conversation
...
class TeamCollaborationService:
    ...
    async def validate_team_memory(
        ...
        try:
            ...
            memory.relevance_score = min(memory.relevance_score + 0.1, 1.0) if is_valid else max(memory.relevance_score - 0.1, 0.0)
        ...
        try:
            ...
            for memory in memories:
                formatted_memories.append({
                    ...
                    "consensus_status": self._calculate_consensus_level(memory.consensus_level).value,
    ...
    async def analyze_team_interaction_patterns(self, team_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            interaction_analysis = {
                "communication_frequency": self._analyze_communication_frequency(conversations),
                "participation_patterns": await self._analyze_participation_patterns(session, team_id, conversations),
                "collaboration_effectiveness": self._analyze_collaboration_effectiveness(conversations),
                "knowledge_sharing": await self._analyze_knowledge_sharing(session, team_id),
                "decision_patterns": await self._analyze_decision_patterns(session, team_id),
                "team_health_indicators": await self._calculate_team_health_indicators(session, team_id)
            }
    ...
    def _analyze_communication_frequency(self, conversations: List[Conversation]) -> Dict[str, Any]:
        ...
        peak_hour = max(hourly_patterns.items(), key=lambda x: x[1])[0] if hourly_patterns else 9
    ...
    async def _analyze_participation_patterns(
        ...
        return {
            ...
            "most_active_member": sorted_participation[0][0] if sorted_participation else None,
            "participation_balance": self._calculate_participation_balance(member_participation)
        }
    
    def _calculate_participation_balance(self, participation: Dict[str, Dict[str, int]]) -> float:
        ...
        variance = sum((x - mean_conversations) ** 2 for x in conversation_counts) / len(conversation_counts)
        
        # Return balance score (higher = more balanced)
        return max(0.0, 1.0 - (variance / max(mean_conversations, 1)))
    
    def _analyze_collaboration_effectiveness(self, conversations: List[Conversation]) -> Dict[str, Any]:
    ...
    async def _analyze_knowledge_sharing(self, session: AsyncSession, team_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_consensus": sum(m.consensus_level for m in memories) / max(len(memories), 1),
            "validated_knowledge_ratio": sum(1 for m in memories if m.consensus_level > 0.5) / max(len(memories), 1)
        }
    
    async def _analyze_decision_patterns(self, session: AsyncSession, team_id: str) -> Dict[str, Any]:
        ...
        if implemented_decisions:
            ...
            avg_implementation_time = sum(implementation_times) / len(implementation_times)
    ...
    async def _calculate_team_health_indicators(self, session: AsyncSession, team_id: str) -> Dict[str, Any]:
        ...
        if memories:
            ...
            high_consensus_ratio = sum(1 for m in memories if m.consensus_level > 0.7) / len(memories)
        else:
            avg_consensus = 0.0
            high_consensus_ratio = 0.0
        
        # Calculate overall health score
        health_score = (avg_consensus * 0.4 + high_consensus_ratio * 0.3 + min(team_size / 5, 1.0) * 0.3)
    ...
    async def update_memory_permissions(
        ...
        try:
            ...
            if not member or member.role not in ["team_lead", "admin"]:
                raise ValueError("User does not have permission to modify memory permissions")
    ...
    async def cross_team_memory_sharing(
        ...
        try:
            ...
            if not source_member or source_member.role not in ["team_lead", "admin"]:
                raise ValueError("User does not have permission to share from source team")
