...
class MemoryService:
        ...
        try:
            ...
            if vector_id:
                ...
                memory_embedding = MemoryEmbedding(
                    ...
                    content_hash=self.vector_db.db.embedding_generator.get_content_hash(content),
                    vector_id=vector_id,
    ...
    async def get_conversation_context(
        ...
        try:
            ...
            recent_decisions = db.query(Decision)\
            ...
            logger.info(f"Generated conversation context with {len(relevant_memories)} memories")
            return context
    ...
    async def update_memory_embedding(
        ...
        try:
            ...
            embedding = db.query(MemoryEmbedding)\
                .filter(
                    MemoryEmbedding.user_memory_id == memory_id if memory_type == "user_memory"
            ...
            if success:
                # Update content hash
                embedding.content_hash = self.vector_db.db.embedding_generator.get_content_hash(memory.content)
    ...
    async def analyze_memory_patterns(
        ...
        try:
            ...
            user_memories = db.query(UserMemory)\
                .filter(
            ...
            analysis = {
                ...
                "most_common_type": max(memory_by_type.items(), key=lambda x: x[1])[0] if memory_by_type else None,
                "most_common_category": max(memory_by_category.items(), key=lambda x: x[1])[0] if memory_by_category else None,
