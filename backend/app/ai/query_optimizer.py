...
class CacheEntry:
    ...
    def is_expired(self) -> bool:
        ...
        return datetime.now(timezone.utc) > self.cached_at + timedelta(
...
class QueryCache:
    ...
    def invalidate_pattern(self, pattern: str):
        ...
        logger.info(
            f"Invalidated {len(to_remove)} cache entries matching pattern: {pattern}"
        )
...
class QueryAnalyzer:
    ...
    async def analyze_query(
        ...
        try:
            ...
            optimization_strategies = self._select_optimization_strategies(
                query, complexity, estimated_time
            )
    ...
    def _analyze_complexity(self, query: str) -> QueryComplexity:
        ...
        for complexity_level, keywords in self.complexity_indicators[
            "keywords"
        ].items():
        ...
        if (
            query_length
            > self.complexity_indicators["length_thresholds"]["very_complex"]
        ):
            length_score = 4
    ...
    def _estimate_processing_time(
        self, query: str, complexity: QueryComplexity, context: Optional[Dict[str, Any]]
    ) -> float:
        ...
        if any(
            word in query.lower() for word in ["comprehensive", "detailed", "thorough"]
        ):
    ...
    def _select_optimization_strategies(
        self, query: str, complexity: QueryComplexity, estimated_time: float
    ...
    def _determine_cache_strategy(
        ...
        if any(
            word in query_lower for word in ["policy", "documentation", "procedure"]
        ):
...
class QueryOptimizer:
    ...
    async def optimize_query_processing(
        ...
        try:
            ...
            if (
                ...
                and analysis.cache_strategy != CacheStrategy.NO_CACHE
            ):
    ...
    def cache_result(
        ...
        if (
            not self.settings["enable_caching"]
            or analysis.cache_strategy == CacheStrategy.NO_CACHE
        ):
            return
    ...
    async def optimize_subquery_processing(
        self, subqueries: List[str], original_context: Optional[Dict[str, Any]] = None
        ...
        try:
            ...
            subquery_analyses.sort(
                key=lambda x: (x[1].priority_score, x[1].complexity.value), reverse=True
            )
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        # Performance trends
        performance_trend = (
            processing_times[-10:] if len(processing_times) >= 10 else processing_times
        )
        ...
        return {
            ...
            "cost_metrics": {
                ...
                "average_cost_per_query": (
                    total_cost / total_queries if total_queries > 0 else 0
                ),
            },
