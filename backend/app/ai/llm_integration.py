...
class LLMManager:
    ...
    def _initialize_models(self):
        ...
        if settings.ANTHROPIC_API_KEY:
            ...
            for config in self.model_configs[ModelProvider.ANTHROPIC]:
                self.providers[ModelProvider.ANTHROPIC][config.model_name] = (
                    ChatAnthropic(
                        model=config.model_name,
                        temperature=config.temperature,
                        max_tokens=config.max_tokens,
        ...
        try:
            ...
            if streaming:
                return self._stream_response(
                    llm, messages, provider, model_name, config
                )
            else:
                ...
                if conversation_id:
                    self._update_conversation_memory(
                        conversation_id, prompt, response.content
                    )

                # Track usage stats
                self._update_usage_stats(
                    provider, model_name, tokens_used, cost_estimate
                )
    ...
    def _calculate_confidence(self, content: str) -> float:
        ...
        if len(content) < 50:
            ...
            for word in ["likely", "probably", "might", "perhaps"]
        ):
    ...
    def _update_usage_stats(
        self, provider: ModelProvider, model: str, tokens: int, cost: float
    ):
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_requests": sum(
                s["total_requests"] for s in self.usage_stats.values()
            ),
