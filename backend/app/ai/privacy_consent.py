...
from app.models.memory import (
    PrivacyConsent, DataRetentionPolicy, DataExportRequest,
...
class PrivacyConsentService:
    ...
    async def grant_consent(
        ...
        try:
            ...
            if expires_in_days:
                ...
                expires_at = datetime.now(timezone.utc) + timedelta(days=self.default_consent_expiry_days)
            ...
            logger.info(f"Granted consent {consent_id} for user {user_id}: {consent_type}/{consent_scope}")
            return consent_id
    ...
    async def withdraw_consent(
        ...
        try:
            ...
            await self._handle_consent_withdrawal(session, user_id, consent_type, consent_scope)
            
            logger.info(f"Withdrew consent for user {user_id}: {consent_type}/{consent_scope}")
            return True
    ...
    async def _anonymize_personal_memories(self, session: AsyncSession, user_id: str) -> None:
    ...
    async def _remove_team_memory_participation(self, session: AsyncSession, user_id: str) -> None:
    ...
    async def _stop_preference_learning(self, session: AsyncSession, user_id: str) -> None:
    ...
    async def _anonymize_analytics_data(self, session: AsyncSession, user_id: str) -> None:
    ...
    async def check_consent_status(
        ...
        try:
            ...
            return {
                ...
                "last_confirmed_at": consent.last_confirmed_at.isoformat() if consent.last_confirmed_at else None
            }
    ...
    async def get_user_consents(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            consents_query = select(PrivacyConsent).where(
                PrivacyConsent.user_id == user_id
    ...
    async def create_data_export_request(
        ...
        try:
            ...
            download_expires_at = datetime.now(timezone.utc) + timedelta(days=7)  # 7-day expiry
            ...
            logger.info(f"Created data export request {export_request.id} for user {user_id}")
    ...
    async def _process_data_export(self, session: AsyncSession, request_id: str) -> None:
        """Process a data export request."""
        try:
            # Get export request
            request_query = select(DataExportRequest).where(DataExportRequest.id == request_id)
    ...
    async def _collect_export_data(
        ...
        for category in export_request.data_categories:
            if category == "conversations":
                conversations = await self._export_conversations(session, user_id, date_start, date_end)
                export_data["conversations"] = conversations
                
            elif category == "user_memories":
                memories = await self._export_user_memories(session, user_id, date_start, date_end)
                export_data["user_memories"] = memories
                
            elif category == "preferences":
                preferences = await self._export_preferences(session, user_id, date_start, date_end)
                export_data["preferences"] = preferences
                
            elif category == "decisions":
                decisions = await self._export_decisions(session, user_id, date_start, date_end)
                export_data["decisions"] = decisions
                
            elif category == "team_memories":
                team_memories = await self._export_team_memories(session, user_id, date_start, date_end)
    ...
    async def _export_team_memories(
        ...
        for memory in team_memories:
            ...
            memory_data = {
                ...
                "memory_type": memory.memory_type,
                "category": memory.category,
                "user_role": "creator" if memory.created_by_user_id == user_id else "validator",
    ...
    async def _generate_csv_export(self, file_path: Path, export_data: Dict[str, Any]) -> None:
    ...
    async def _generate_xml_export(self, file_path: Path, export_data: Dict[str, Any]) -> None:
        ...
        for category, data in export_data.items():
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    item_elem = ET.SubElement(category_elem, category[:-1] if category.endswith('s') else 'item')
    ...
    async def get_export_status(self, request_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "processing_started_at": export_request.processing_started_at.isoformat() if export_request.processing_started_at else None,
