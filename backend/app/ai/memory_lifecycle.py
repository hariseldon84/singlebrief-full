...
class MemoryLifecycleManager:
        ...
        try:
            ...
            for policy, days in retention_policies.items():
                ...
                if policy == "delete_on_request":
                    ...
                    user_memory_query = user_memory_query.filter(
                        UserMemory.memory_metadata.op('->>')('deletion_requested') == 'true'
                    )
                ...
                if policy == "delete_on_request":
                    team_memory_query = team_memory_query.filter(
                        TeamMemory.team_metadata.op('->>')('deletion_requested') == 'true'
                    )
            ...
            logger.info(f"Applied expiration policies: {expired_counts['total_items']} items processed")
            return result
    ...
    async def _compress_conversation_messages(self, db: Session, conversation_id: str):
        ...
        try:
            ...
            summary_content = f"Conversation summary: {len(messages)} messages exchanged"
    ...
    async def cleanup_orphaned_embeddings(
        ...
        try:
            ...
            orphaned_embeddings = db.query(MemoryEmbedding).filter(
                and_(
                    or_(
                        and_(
                            MemoryEmbedding.user_memory_id.isnot(None),
                            ~db.query(UserMemory).filter(
                                UserMemory.id == MemoryEmbedding.user_memory_id
                            ).exists()
                        ),
                        and_(
                            MemoryEmbedding.team_memory_id.isnot(None),
                            ~db.query(TeamMemory).filter(
                                TeamMemory.id == MemoryEmbedding.team_memory_id
                            ).exists()
                        )
                    )
                )
            ...
            logger.info(f"Cleaned up {cleanup_counts['orphaned_embeddings']} orphaned embeddings")
            return result
    ...
    async def optimize_memory_storage(
        ...
        try:
            ...
            duplicates_merged = await self._merge_duplicate_memories(db, organization_id)
            optimization_results["duplicates_merged"] = duplicates_merged

            # Archive low-importance, old memories
            archived_count = await self._archive_low_importance_memories(db, organization_id)
            ...
            optimization_results["storage_freed_mb"] = total_items * 0.5  # Rough estimate
            ...
            for group_memories in user_groups.values():
                if len(group_memories) > 1:
                    ...
                    for duplicate in group_memories[1:]:
                        ...
                        if self._calculate_content_similarity(primary_memory.content, duplicate.content) > 0.8:
    ...
    async def _merge_memory_metadata(self, primary: UserMemory, duplicate: UserMemory):
        ...
        try:
            ...
            primary_meta["merged_memories"].append({
                ...
                "original_content": duplicate.content[:100] + "..." if len(duplicate.content) > 100 else duplicate.content
            })
            
            # Update importance score to average
            primary.importance_score = (primary.importance_score + duplicate.importance_score) / 2
    ...
    async def _archive_low_importance_memories(
        ...
        try:
            ...
            for memory in low_importance_memories.all():
                ...
                memory.memory_metadata["archived_at"] = datetime.now(timezone.utc).isoformat()
                memory.updated_at = datetime.now(timezone.utc)
                archived_count += 1
    ...
    async def _mark_unused_memories(
        ...
        try:
            ...
            for memory in unused_memories.all():
                ...
                memory.memory_metadata["marked_unused_at"] = datetime.now(timezone.utc).isoformat()
                memory.updated_at = datetime.now(timezone.utc)
                marked_count += 1
    ...
    async def generate_memory_usage_report(
        ...
        try:
            ...
            active_user_memories = user_memory_query.filter(UserMemory.is_active == True).count()
            total_team_memories = team_memory_query.count()
            active_team_memories = team_memory_query.filter(TeamMemory.is_active == True).count()
            total_conversations = conversation_query.count()
            archived_conversations = conversation_query.filter(Conversation.is_archived == True).count()

            # Memory type distribution
            memory_types = db.query(
                UserMemory.memory_type,
                func.count(UserMemory.id).label('count')
            ).filter(UserMemory.is_active == True).group_by(UserMemory.memory_type).all()

            # Category distribution
            categories = db.query(
                UserMemory.category,
                func.count(UserMemory.id).label('count')
            ).filter(UserMemory.is_active == True).group_by(UserMemory.category).all()
            ...
            report = {
                ...
                "health_metrics": {
                    "active_memory_ratio": active_user_memories / total_user_memories if total_user_memories > 0 else 0,
                    "archive_ratio": archived_conversations / total_conversations if total_conversations > 0 else 0,
                    "avg_importance": db.query(func.avg(UserMemory.importance_score)).filter(
                        UserMemory.is_active == True
                    ).scalar() or 0
                }
            }

            logger.info(f"Generated memory usage report for organization {organization_id}")
            return report
