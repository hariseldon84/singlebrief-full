...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        self.token_splitter = TokenTextSplitter(chunk_size=512, chunk_overlap=50)

    def process_document(
        self, content: str, document_type: DocumentType, metadata: Dict[str, Any]
    ) -> List[Document]:
        """Process document into chunks with metadata"""

        # Choose splitter based on document type
        if document_type in [
            DocumentType.CODE_REPOSITORY,
...
class VectorStore:
    ...
    async def create_index(
        self, index_name: str, documents: List[Document], index_type: str = "faiss"
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Created vector index '{index_name}' with {len(documents)} documents"
            )
            return True
    ...
    async def similarity_search(
        self, index_name: str, query: str, k: int = 5, score_threshold: float = 0.0
...
class RAGPipeline:
    ...
    async def index_document(
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"Indexed document: {len(documents)} chunks for {document_type.value}"
                )
    ...
    async def retrieve_context(
        ...
        try:
            ...
            if retrieval_type == RetrievalType.SEMANTIC:
                ...
                retrieved_contexts = self._deduplicate_contexts(all_contexts)[
                    :max_contexts
                ]
            ...
            total_chunks_searched = sum(
                len(self.document_stores[dt]) for dt in document_types
            )
    ...
    async def _keyword_retrieval(
        ...
        for doc_type in document_types:
            ...
            for doc in documents:
                doc_keywords = self.document_processor.extract_keywords(
                    doc.page_content
                )
    ...
    async def _generate_query_variations(self, query: str) -> List[str]:
        ...
        try:
            response = await self.llm_manager.generate_response(
                ...
                system_message="Generate query variations only.",
            )
    ...
    async def _compress_contexts(
        ...
        try:
            response = await self.llm_manager.generate_response(
                prompt,
                QueryType.ANALYSIS,
                system_message="Rank and analyze context relevance.",
            )
    ...
    def _update_retrieval_stats(
        self, retrieval_type: RetrievalType, processing_time: int
    ):
        ...
        if method not in self.retrieval_stats["retrieval_methods"]:
            self.retrieval_stats["retrieval_methods"][method] = {
                "count": 0,
                "avg_time": 0.0,
            }
        ...
        method_stats["avg_time"] = (
            (method_avg * (method_stats["count"] - 1)) + processing_time
        ) / method_stats["count"]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "document_counts": {
                dt.value: len(docs) for dt, docs in self.document_stores.items()
            },
            "vector_stores": list(self.vector_store.vector_stores.keys()),
            "total_indexed_chunks": sum(
                len(docs) for docs in self.document_stores.values()
            ),
        }
