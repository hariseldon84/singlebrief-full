...
class Query(Base):
    ...
    briefs: Mapped[List["Brief"]] = relationship(
        "Brief", back_populates="query", cascade="all, delete-orphan"
    )
...
class Brief(Base):
    ...
    template_id: Mapped[Optional[str]] = mapped_column(
        String(100), comment="Template used to generate this brief"
    )
    ...
    sections: Mapped[List[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, comment="Structured sections of the brief"
    )
    key_insights: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Key insights and takeaways"
    )
    action_items: Mapped[Optional[List[Dict[str, Any]]]] = mapped_column(
        JSONB, comment="Recommended actions and next steps"
    )
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="briefs"
    )
    delivery_logs: Mapped[List["BriefDeliveryLog"]] = relationship(
        "BriefDeliveryLog", back_populates="brief", cascade="all, delete-orphan"
    )
...
class BriefDeliveryLog(Base):
    ...
    brief_id: Mapped[str] = mapped_column(
        UUID(as_uuid=False), ForeignKey("briefs.id", ondelete="CASCADE"), nullable=False
    )
    ...
    recipient_type: Mapped[str] = mapped_column(
        String(20), default="user", comment="Type: user, team, webhook, external_system"
    )
    ...
    recipient_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Additional recipient metadata"
    )
    ...
    delivery_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Channel-specific delivery metadata"
    )
...
class BriefTemplate(Base):
    ...
    template_schema: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="Template structure and schema definition"
    )
    sections: Mapped[List[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, comment="Template sections and their configuration"
    )

    # Generation instructions
    generation_prompt: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Prompt template for AI generation"
    )
    data_requirements: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Required data sources and types"
    )
    output_format: Mapped[str] = mapped_column(
        String(20), default="markdown", comment="Output format: markdown, html, json"
    )
    ...
    default_time_range_hours: Mapped[int] = mapped_column(
        default=24, comment="Default time range in hours"
    )
    priority_sources: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Priority data sources for this template"
    )
    ...
    # Relationships
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="brief_templates"
    )
    created_by: Mapped["User"] = relationship(
        "User", back_populates="created_brief_templates"
    )
