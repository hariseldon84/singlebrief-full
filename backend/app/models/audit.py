...
class AuditLog(Base):
    ...
    resource_id: Mapped[Optional[str]] = mapped_column(
        String(255), comment="ID of the specific resource affected"
    )
    ...
    old_values: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Previous values for update operations"
    )
    new_values: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="New values for create/update operations"
    )
    ...
    risk_level: Mapped[str] = mapped_column(
        String(20), default="low", comment="Risk level: low, medium, high, critical"
    )
    compliance_flags: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Compliance framework flags: gdpr, ccpa, hipaa, sox, etc."
    )
    retention_period_days: Mapped[int] = mapped_column(
        default=2555, comment="Days to retain this audit record"  # 7 years default
    )
...
class ConsentRecord(Base):
    ...
    purpose: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Specific purpose for consent"
    )
    scope: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Scope: user, team, organization, global"
    )
    ...
    data_categories: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Categories of data covered by this consent"
    )
    processing_activities: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Specific processing activities covered"
    )
    third_parties: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Third parties data may be shared with"
    )

    # Geographic and temporal scope
    geographic_scope: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Geographic regions where consent applies"
    )
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="consent_records"
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "user_id", "consent_type", "purpose", name="uq_user_consent_purpose"
        ),
        CheckConstraint(
...
class DataAccessLog(Base):
    ...
    # Context and purpose
    purpose: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Purpose of data access"
    )
    legal_basis: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Legal basis for data access"
    )
    ...
    query_details: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Details of the query or access pattern"
    )
    data_categories: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Categories of data accessed"
    )
    ...
    user: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[user_id], back_populates="data_access_logs"
    )
    accessed_by: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[accessed_by_user_id]
    )
    organization: Mapped[Optional["Organization"]] = relationship(
        "Organization", back_populates="data_access_logs"
    )
...
class PrivacySetting(Base):
    ...
    conversation_retention_days: Mapped[Optional[int]] = mapped_column(
        default=365, comment="Days to retain conversations, null for indefinite"
    )
    ...
    data_export_format: Mapped[str] = mapped_column(
        String(20), default="json", comment="Preferred export format: json, csv, pdf"
    )

    # Custom privacy preferences
    custom_settings: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Custom privacy settings and preferences"
    )
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="privacy_settings"
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "user_id", "organization_id", name="uq_user_org_privacy_settings"
        ),
        CheckConstraint(
...
class GDPRRequest(Base):
    ...
    data_categories: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Specific data categories requested"
    )
    ...
    priority: Mapped[str] = mapped_column(
        String(20), default="standard", comment="Priority: low, standard, high, urgent"
    )
    ...
    response_format: Mapped[str] = mapped_column(
        String(20), default="json", comment="Response format: json, csv, pdf, email"
    )
    ...
    user: Mapped["User"] = relationship(
        "User", foreign_keys=[user_id], back_populates="gdpr_requests"
    )
    handled_by: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[handled_by_user_id]
    )
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="gdpr_requests"
    )
...
class SecurityEvent(Base):
    ...
    threat_indicators: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Threat indicators and IOCs"
    )
    risk_score: Mapped[float] = mapped_column(
        default=0.0, comment="Risk score 0.0-10.0"
    )
    confidence_score: Mapped[float] = mapped_column(
        default=0.5, comment="Confidence in threat assessment 0.0-1.0"
    )
    ...
    response_actions: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, comment="Actions taken in response to the event"
    )
    assigned_to: Mapped[Optional[str]] = mapped_column(String(255))

    # Additional data
    event_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, comment="Additional event metadata and context"
    )
    ...
    # Relationships
    user: Mapped[Optional["User"]] = relationship(
        "User", back_populates="security_events"
    )
    organization: Mapped[Optional["Organization"]] = relationship(
        "Organization", back_populates="security_events"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "risk_score >= 0.0 AND risk_score <= 10.0", name="check_risk_score_range"
        ),
        CheckConstraint(
